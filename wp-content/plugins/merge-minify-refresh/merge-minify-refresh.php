	...
	public function mmr_files_callback() {
		...
		if(count($files) > 0) {
			...
			foreach($files as $file) {
				...
				{
					$accessed = file__get_contents($script_path.'.accessed');
				...
				if(!isset($handles[$i]['handle'])) {
					...
					if(!file_exists($full_path) && !$min_exists) {
						...
						foreach( $handles[$i]['handles'] as $handle ) {
							...
							// Remove the BOM
							$contents .= preg_replace("/^\xEF\xBB\xBF/", '', file__get_contents($this->root.$script_path)) . $script_line_end;
